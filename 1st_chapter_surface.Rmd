...
pkgs <- c("tidyverse", "phyloseq", 'vegan', "geosphere","skimr", 'tidymodels', 'tidytext') # Load packages
invisible(sapply(pkgs,require, character = TRUE))

# library(CoDaSeq)
# library(zCompositions)

# source codes from microbiomeSeq
# load my functions
...
out.mh <- c("ocean", "G-river", "K-river")
MH_env.surf <-  MH_env %>% rownames_to_column(var = "sample") %>%
  filter(Depth == 3) %>% filter(!Type %in% out.mh) %>%
...
```{r}
# skimr package 
summary_df <- MH_env %>% filter(Depth == 3) %>%
  group_by(region) %>%
  select (Temperature:Carbon,AOB_copies, AOA_copies, nosZ_copies, Chao1,Shannon) %>%
  skimr::skim()

summary_df %>% 
  select(skim_variable, region, numeric.mean:numeric.p0, numeric.p100) %>%
  rename(variable = skim_variable) %>%
  rename_at(vars(starts_with("numeric")), ~str_replace(.,'numeric.','')) %>%
  rename(min = p0) %>%
  rename(max = p100) %>%
  pivot_wider(names_from = region, values_from  = mean:max) %>%
  select(variable,ends_with("north"), ends_with("central"), ends_with("south"))
```
...
stats.sum.surf <- data_long %>% 
  ...
  select(env_par, tidy) %>% 
  unnest(tidy)
write.csv(stats.sum.surf, "C:/Users/rrocha/Documents/R/phd/2016/output/data/anova_env_summary_surf.csv")
...
stats.sum.surf.Tukey <- data_long %>% 
  ...
  select(env_par, tidy) %>% 
  unnest(tidy)
write.csv(stats.sum.surf.Tukey, "C:/Users/rrocha/Documents/R/phd/2016/output/data/anova_env_summary_surf_tukey.csv")
```
...
perm.surf <- vegan::adonis(dist(t(d.clr.abund.filt.mh.surf), method='euclidean') ~ region, 
...
fit <- kmeans(mydata, 4) # 6 cluster solution
# get cluster means
...
pseq.filt.clr.surf <- phyloseq(otu_table(as.matrix(d.clr.abund.filt.mh.surf), taxa_are_rows = TRUE), 
                          tax_table(as.matrix(TAX.filt.mh.surf)),sample_data(MH_env.surf) )
MH.clr.ord <- ordinate(pseq.filt.clr.surf , method = "RDA")
p.samp.ord = plot_ordination(pseq.filt.clr.surf , MH.clr.ord, type="samples", color = "region")
print(p.samp.ord)
p.otu.ord = plot_ordination(pseq.filt.clr.surf , MH.clr.ord, type="taxa", color = "Phylum", 
                            title = "PCA on Aitichson Distance - Phylum")
p.otu.ord + facet_wrap(~Class)


# unconstrained ordination (RDA)
# prepare the data
MH_env.surf.na <- MH_env.surf %>% drop_na()
X <- select_if(MH_env.surf.na,is.numeric) %>% 
  dplyr::select(DO, Salinity, Temperature, Nitrogen, Carbon, CN, TSSmean)
X <- decostand(X, method='standardize')
samples.na <- MH_env.surf %>% drop_na()
Y <- d.clr.abund.filt.mh.surf %>% t() %>% as.data.frame() %>% 
...
vars.ordistep <- gsub('^. ', '', rownames(step.env$anova)) # TSS p.value = 0.025 (no significant)
...
env_fact = MH_env.surf$nosZ_copies
